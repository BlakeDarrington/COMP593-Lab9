...
def handle_get_info_btn_click():
    ...
    if poke_info is None:
        err_msg= f'Unable to get the information from the PokeAPI for {poke_name}.'
    ...
    lbl_type_value['text'] = ', '.join([t['type']['name'].title() for t in poke_info['types']])
...
# Add widgets bottom right frame
lbl_height = ttk.Label(frm_btm_left, text='Height:')
lbl_height.grid(row=0, column=0, sticky=E, pady=(5, 15))

lbl_height_value = ttk.Label(frm_btm_left, text='TBD')
lbl_height_value.grid(row=0, column=1, pady= (5, 15))

lbl_weight = ttk.Label(frm_btm_left, text='Weight')
lbl_weight.grid(row=1, column=0, sticky=E, pady=(5, 15))

lbl_weight_value = ttk.Label(frm_btm_left, text='TBD')
lbl_weight_value.grid(row=1, column=1, pady= (5, 15))

lbl_type = ttk.Label(frm_btm_left, text='Type: ')
lbl_type.grid(row=2, column=0, sticky=E, pady=(5, 15))

lbl_type_value = ttk.Label(frm_btm_left, text='TBD')
lbl_type_value.grid(row=2, column=1, pady=(5, 15))
...
prg_hp.grid(row=0, column=1, padx=20, pady=5)
...
prg_attack.grid(row=1, column=1, padx=20, pady=5)
...
prg_defense.grid(row=2, column=1, padx=20, pady=5)
...
prg_sp_attack.grid(row=3, column=1, padx=20, pady=5)
...
prg_sp_defense.grid(row=4, column=1, padx=20, pady=5)
...
prg_speed.grid(row=5, column=1, padx=20, pady=5)
